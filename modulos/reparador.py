import os
import ast
import shutil
import datetime

BASE = "C:/IA/AGENTE/MECANICO"

def leer_archivo(ruta):
    try:
        with open(ruta, "r", encoding="utf-8", errors="ignore") as f:
            return f.read()
    except Exception as e:
        return f"ERROR: {e}"

def escribir_archivo(ruta, contenido):
    try:
        with open(ruta, "w", encoding="utf-8") as f:
            f.write(contenido)
        return True
    except Exception as e:
        return False

def hacer_backup(ruta):
    fecha = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    backup = os.path.join(BASE, "memoria/backups", os.path.basename(ruta) + f".backup_{fecha}")
    shutil.copy2(ruta, backup)
    return backup

def validar_python(codigo):
    try:
        ast.parse(codigo)
        return True, "OK"
    except SyntaxError as e:
        return False, f"SyntaxError linea {e.lineno}: {e.msg}"

def extraer_codigo(texto):
    if "```python" in texto:
        inicio = texto.find("```python") + 9
        fin = texto.find("```", inicio)
        return texto[inicio:fin].strip()
    elif "```" in texto:
        inicio = texto.find("```") + 3
        fin = texto.find("```", inicio)
        return texto[inicio:fin].strip()
    return texto.strip()

def reparar_archivo(ruta, preguntar_fn):
    if not os.path.exists(ruta):
        return f"ERROR: Archivo no encontrado: {ruta}"
    codigo_original = leer_archivo(ruta)
    if codigo_original.startswith("ERROR"):
        return codigo_original
    log = []
    log.append(f"Reparando: {ruta}")
    prompt = f"""Sos un experto en Python. Analizá este codigo y devolvé SOLO el codigo corregido, sin explicaciones.
Corregi todos los errores, bugs y problemas de calidad que encuentres.
Mantene la funcionalidad original intacta.
Devolvé SOLO el codigo Python completo y corregido entre triple backticks.

Codigo a reparar:
```python
{codigo_original[:4000]}
```

Devolvé SOLO el codigo corregido:"""
    log.append("Consultando IA para reparacion...")
    codigo_nuevo = preguntar_fn(prompt)
    codigo_nuevo = extraer_codigo(codigo_nuevo)
    if not codigo_nuevo:
        return "ERROR: La IA no devolvio codigo"
    valido, msg = validar_python(codigo_nuevo)
    if not valido:
        return f"ERROR: Codigo generado invalido: {msg}"
    log.append("Codigo valido, haciendo backup...")
    backup = hacer_backup(ruta)
    log.append(f"Backup: {backup}")
    ok = escribir_archivo(ruta, codigo_nuevo)
    if not ok:
        return "ERROR: No se pudo escribir el archivo"
    log.append("Archivo reparado exitosamente")
    try:
        import sys
        sys.path.insert(0, BASE)
        from modulos import git_manager
        git_result = git_manager.commit_automatico(f"MECANICO reparacion: {os.path.basename(ruta)}")
        log.append(f"Git commit: {git_result}")
    except Exception as e:
        log.append(f"Git warning: {e}")
    log.append(f"Lineas originales: {len(codigo_original.splitlines())}")
    log.append(f"Lineas nuevas: {len(codigo_nuevo.splitlines())}")
    return "\n".join(log)

def ejecutar(accion, texto):
    palabras = texto.split()
    ruta = palabras[-1] if len(palabras) > 1 else ""
    if not ruta or not os.path.exists(ruta):
        return "ERROR: Especifica la ruta. Ej: reparar C:/ruta/archivo.py"
    from mecanico import preguntar
    return reparar_archivo(ruta, preguntar)
